...
void CimagePro20190802View::OnDraw(CDC* pDC)
{
	...
	for (y = 0; y < 256; y++) {
		for (x = 0; x < 256; x++) {
			pDC->SetPixel(256 + 20 + x, y, RGB(pDoc->inputImage2[y][x], pDoc->inputImage2[y][x], pDoc->inputImage2[y][x]));
		}
	}
...
void CimagePro20190802View::OnPixelHistEqu()
{
	...
	for (y = 0; y < 256; y++) {
		for (x = 0; x < 256; x++) {
			...
			pDoc->resultImage1[y][x] = (float)sum[k] / N * 255;
		}
	}
...
void CimagePro20190802View::OnPixelContrastStretching()
{
	...
	for (y = 0; y < 256; y++) {
		for (x = 0; x < 256; x++) {
			...
			pDoc->resultImage2[x][y] = (float)(p - min) / (max - min) * 255;
		}
	}
	...
	for (y = 0; y < 256; y++) {
		for (x = 0; x < 256; x++) {
			if (pDoc->inputImage[y][x] > thresh)
				pDoc->resultImage3[y][x] = 255;
			else
				pDoc->resultImage3[y][x] = 0;
		}
	}
...
void CimagePro20190802View::OnPixelTwoImageAdd()
{
	...
	if (dlg.DoModal() == IDOK) {
		...
		for (y = 0; y < 256; y++) {
			for (x = 0; x < 256; x++) {
				pDoc->resultImage1[y][x] = 0.5 * pDoc->inputImage[y][x] + 0.5 * pDoc->inputImage2[y][x];
			}
		}
